...
// TODO: Include the associate buttons to to visualizers to include stop method
...
const expChart = new LineChart('#exp-chart');
...
function clearVisualizers() {
  ...
  UI.setButtonDefault(UI.nodes.amplitudeHeatmapBtn);
  UI.setButtonDefault(UI.nodes.phaseHeatmapBtn);
  UI.setButtonDefault(UI.nodes.gatesHeatmapBtn);
  UI.setButtonDefault(UI.nodes.expChartBtn);
  UI.setButtonDefault(UI.nodes.d3PlotBtn);
  UI.setHeaderDefault();
  UI.setAsidesDefault();
}
...
function wireFloatingActionButtons() {
  ...
  UI.nodes.amplitudeHeatmapBtn.addEventListener('click', () => {
    ...
    if (UI.isButtonActive(ampHeatmapBtn)) {
      UI.hideUI(UI.nodes.amplitudeHeatmapContainer);
      UI.setButtonDefault(ampHeatmapBtn);
      ampHeatmap.clear();
    } else {
      UI.showUI(UI.nodes.amplitudeHeatmapContainer);
      UI.setButtonActive(ampHeatmapBtn);
  ...
  UI.nodes.phaseHeatmapBtn.addEventListener('click', () => {
    ...
    if (UI.isButtonActive(phaseHeatmapBtn)) {
      UI.hideUI(UI.nodes.phaseHeatmapContainer);
      UI.setButtonDefault(phaseHeatmapBtn);
      phaseHeatmap.clear();
    } else {
      UI.showUI(UI.nodes.phaseHeatmapContainer);
      UI.setButtonActive(phaseHeatmapBtn);
  ...
  UI.nodes.gatesHeatmapBtn.addEventListener('click', () => {
    ...
    if (UI.isButtonActive(gatesHeatmapBtn)) {
      UI.hideUI(UI.nodes.gatesHeatmapContainer);
      UI.setButtonDefault(gatesHeatmapBtn);
      gatesHeatmap.clear();
    } else {
      UI.showUI(UI.nodes.gatesHeatmapContainer);
      UI.setButtonActive(gatesHeatmapBtn);
  ...
  UI.nodes.expChartBtn.addEventListener('click', () => {
    ...
    if (UI.isButtonActive(expChartBtn)) {
      UI.hideUI(UI.nodes.expChartContainer);
      UI.setButtonDefault(expChartBtn);
      expChart.clear();
    } else {
      UI.showUI(UI.nodes.expChartContainer);
      UI.setButtonActive(expChartBtn);
      expChart.init();
    }
  });
